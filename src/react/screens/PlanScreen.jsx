...
function PlanCanvas({ plan, tool, selected, setSelected, commitPlan, polygonDraft, setPolygonDraft, finishPolygon, issues, viewportZoom, onViewportZoom, viewportPan = { x: 0, y: 0 }, onViewportPan, onCreated, onSelected }) {
  ...
  return <svg ref={svgRef} className={`plan-svg tool-${tool}`} viewBox={`0 0 ${VIEW.width} ${VIEW.height}`} role="img" aria-label="Редактор плана дома" onPointerDownCapture={pointerDownCapture} onPointerDown={canvasDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerLeave={() => { if (!gestureRef.current) setHoverSnap(null); }} onPointerCancel={(event) => { panGestureRef.current = null; finishPointer(event); setGesture(null); }}>
    ...
    {(shownPlan.walls || []).map((wall) => { const q = line(wall); const selectedNow = selected?.type === 'wall' && selected.id === wall.id; return <g key={wall.id} className="planner-object wall-band" onPointerDown={(event) => objectDown(event, 'wall', wall.id)}><line className={`wall-band-base standalone-wall ${selectedNow ? 'selected' : ''}`} x1={q.a.x} y1={q.a.y} x2={q.b.x} y2={q.b.y} /><line className="wall-band-hatch" x1={q.a.x} y1={q.a.y} x2={q.b.x} y2={q.b.y} /><line className="wide-hit" x1={q.a.x} y1={q.a.y} x2={q.b.x} y2={q.b.y} />{selectedNow ? [q.a, q.b].map((point, index) => <circle key={index} className="endpoint-handle" cx={point.x} cy={point.y} r="7" onPointerDown={(event) => objectDown(event, 'wall', wall.id, { kind: 'endpoint', index })} />) : null}</g>; })}
...
export default function PlanScreen({ onNavigate }) {
  ...
  const toolHint = tool === 'select' ? 'Выберите и перетащите объект. Окна, двери, ворота и линии обвязки можно двигать.' : tool === 'polygon' ? 'Ставьте углы комнаты. После третьей точки щёлкните по первой точке — контур замкнётся автоматически.' : tool === 'dimension' ? 'Нажмите первую точку, затем вторую. Размер появится между ними и привяжется к узлам/сетке.' : tool === 'pileRow' ? 'Начало и конец прилипают к узлам. Зелёный — точно по горизонтали или вертикали, красный — есть отклонение.' : tool === 'bindingLine' ? 'Протяните направляющую обвязки от узла до узла. Зелёный — точная ось, красный — отклонение.' : ['window', 'door', 'gap', 'pile'].includes(tool) ? 'Щёлкните место установки. Тип гаражных ворот выбирается в параметрах двери.' : 'Зажмите кнопку мыши и протяните объект. Зелёный — горизонталь или вертикаль, красный — отклонение.';
